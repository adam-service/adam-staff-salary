...
function getAllData(){
  ...
  // 確定スナップショット（確定月の凍結報酬。1行=1ゲスト×1月、dataはJSON）
  const snSh=getOrCreateSheet('確定スナップショット',['month','clientId','data']);
  const snRows=sheetToArray(snSh);
  ...
  snRows.forEach(r=>{
    ...
    if(!r.data)return;
    try{
      if(!lockedSnapshots[mk])lockedSnapshots[mk]={};
      lockedSnapshots[mk][cid]=JSON.parse(r.data);
    }catch(e){}
  });
...
function saveAllData(D){
  ...
  // Locked Snapshots（確定スナップショット）
  // 1行=1ゲスト×1月、dataはJSON。
  // ガード: 受信した lockedSnapshots が空のときはシートを消さない
  // （旧バージョンの端末が空送信してもクラウドの凍結値を消さないため）。
  const snData=[];
  if(D.lockedSnapshots){
    Object.keys(D.lockedSnapshots).forEach(mk=>{
      Object.keys(D.lockedSnapshots[mk]||{}).forEach(cid=>{
        snData.push([mk,cid,JSON.stringify(D.lockedSnapshots[mk][cid])]);
      });
    });
  }
  if(snData.length>0){
    const snSh=getOrCreateSheet('確定スナップショット',['month','clientId','data']);
    if(snSh.getLastRow()>1)snSh.getRange(2,1,snSh.getLastRow()-1,3).clearContent();
    snSh.getRange(2,1,snData.length,3).setNumberFormat('@');
    snSh.getRange(2,1,snData.length,3).setValues(snData);
  }
}
